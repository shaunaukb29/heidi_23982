model = dict(
    type='MaskRCNN',
    backbone=dict(
        type='ResNet', depth=101, num_stages=4, out_indices=(0, 1, 2, 3),
    ...
    roi_head=dict(
        ...
        bbox_head=dict(
            type='Shared2FCBBoxHead', in_channels=256, fc_out_channels=1024, roi_feat_size=7, num_classes=6,
            bbox_coder=dict(type='DeltaXYWHBBoxCoder', target_means=[0.0, 0.0, 0.0, 0.0], target_stds=[0.1, 0.1, 0.2, 0.2]),
            reg_class_agnostic=False, loss_cls=dict(type='FocalLoss', use_sigmoid=True, loss_weight=1.0, gamma=2.0, alpha=0.5), loss_bbox=dict(type='L1Loss', loss_weight=1.0)),
        ...
        mask_head=dict(type='FCNMaskHead', num_convs=4, in_channels=256, conv_out_channels=256, num_classes=6, loss_mask=dict(type='CrossEntropyLoss', use_mask=True, loss_weight=1.0))))
...
data = dict(samples_per_gpu=1, workers_per_gpu=2, test=dict(type='CocoDataset', pipeline=[
